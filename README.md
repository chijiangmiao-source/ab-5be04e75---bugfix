...
全部候选始终在一次全局区间 DP 中求解，不按连通分量拆分——拆分会丢失跨分量
的几何约束（跨度包含但弧互相交叉的两组候选会被错误地各自取局部最优后合并）。

区间非交叉匹配（允许嵌套），状态为区间 `[i,j)`：

- inside DP：`跳过 i` 或 `i 与 k 配对`（内部 `[i+1,k)` 与外部 `[k+1,j)` 独立），
  非交叉性由该规则天然保证：与 `(i,k)` 交叉的弧不可能与它同时出现；
  同时维护最大对数、最小残差、任意精度方案数，以及字典序最小 id 序列；
...
1. 代码测试（89 项，含 60 组随机输入与暴力枚举的方案数/规范解/分类对照、
   72 击中/50 候选稀疏族的全局几何约束与多种录入顺序无关性、
   56 候选跨过旧分解阈值的暴力回归）；
2. 构建检查（语法编译、模块导入、镜像内关键文件齐备）；
3. API/HTTP 冒烟（健康路径、嵌套同优、交叉低价诱饵、空候选、非法引用、
   稀疏候选族精确目标与真实接口顺序无关性、重复端点对、位置冲突、规模越界、
   未知路径）。
