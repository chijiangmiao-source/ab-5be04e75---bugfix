...
大批量候选（≥48 条）先按几何相关性分治：端点连通且跨度相交（交错或包含）
的候选归入同一分量，仅当分量跨度两两不交时才独立求解——此时分量间既不可能
交叉也不可能嵌套，目标可加、方案数可乘、规范序列可按位置拼接。
...
1. 代码测试（87 项，含 60 组随机输入与暴力枚举的方案数/规范解/分类对照，
   以及 50 条稀疏候选族的全局几何回归与顺序无关性检查）；
2. 构建检查（语法编译、模块导入、镜像内关键文件齐备）；
3. API/HTTP 冒烟（健康路径、嵌套同优、交叉低价诱饵、空候选、非法引用、
   重复端点对、位置冲突、规模越界、未知路径，以及稀疏候选族的精确目标、
   规范解非交叉与候选录入顺序无关性）。
